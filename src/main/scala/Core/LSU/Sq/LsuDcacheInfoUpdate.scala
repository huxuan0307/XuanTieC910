...
class LsuDcacheInfoUpdateIn extends Bundle with DCacheConfig{
  ...
  val dcacheDirtyDin     = Flipped(ValidIO(Vec(WAYS, new DcacheDirtyDataEn)))
  val dcacheDirtyGwen    = Bool()
  val dcacheDirtyWen     = Flipped(ValidIO(Vec(WAYS, new DcacheDirtyDataEn)))
  val dcacheIdx          = Bool()
  val dcacheTagDin       = Vec(WAYS,Flipped(ValidIO(UInt((TAG_WIDTH).W))))
  val dcacheTagGwen      = Bool()
  val dcacheTagWen       = Vec(WAYS,Bool())
  val originDcacheDirty  = Bool()
  val originDcacheShare  = Bool()
  val originDcacheValid  = Bool()
...
class LsuDcacheInfoUpdate extends Module with DCacheConfig{
  ...
  val compare_dcwp_hit_idx = io.in.compareDcwpAddr(OFFSET_WIDTH+INDEX_WIDTH-1,INDEX_WIDTH-1) === io.in.dcacheIdx
  ...
  compare_dcwp_hit_dirty_din := Mux(io.in.originDcacheWay ,io.in.dcacheDirtyDin.bits(1), io.in.dcacheDirtyDin.bits(0))
  val compare_dcwp_hit_dirty_wen = WireInit(0.U.asTypeOf(new DcacheDirtyDataEn))
  compare_dcwp_hit_dirty_wen := Mux(io.in.originDcacheWay ,io.in.dcacheDirtyWen.bits(1), io.in.dcacheDirtyWen.bits(0))
  val compare_dcwp_hit_up_vld = Mux(io.in.originDcacheWay , io.in.originDcacheValid, compare_dcwp_hit_idx)
  // choose dcache or sq by Wen
  // MESI?
  val compare_dcwp_hit_dirty  = Mux(compare_dcwp_hit_dirty_wen.dirty , compare_dcwp_hit_dirty_din.dirty, io.in.originDcacheDirty)
  val compare_dcwp_hit_share  = Mux(compare_dcwp_hit_dirty_wen.share , compare_dcwp_hit_dirty_din.share, io.in.originDcacheShare)
  val compare_dcwp_hit_valid  = Mux(compare_dcwp_hit_dirty_wen.valid , compare_dcwp_hit_dirty_din.valid, io.in.originDcacheValid)
  //---------------update if dcache miss----------------------
  //dcache set&way inst will NOT APPEAR dcache miss update
  val compare_dcwp_miss_up_pre = io.in.dcacheDirtyGwen && !io.in.compareDcwpSwInst && !io.in.originDcacheValid
  ...
  for(i<- 0 until WAYS){
    compare_dcwp_miss_up_way_sel(i) := io.in.dcacheDirtyDin.bits(i).valid && io.in.dcacheTagWen(i) &&
      (compare_dcwp_tag === io.in.dcacheTagDin(i).bits)
  ...
  compare_dcwp_miss_dirty_din := Mux(compare_dcwp_miss_up_way_sel(1) ,io.in.dcacheDirtyDin.bits(1), io.in.dcacheDirtyDin.bits(0))
  ...
  val compare_dcwp_sw_up_vld = io.in.dcacheDirtyGwen && io.in.compareDcwpSwInst && compare_dcwp_hit_idx
  //---------------------select-------------------------------
  ...
  val compare_dcwp_hit_sel = io.in.originDcacheValid || compare_dcwp_sw_up_vld
  ...
  io.out.updateDcacheDirty := Mux(compare_dcwp_update_vld,update_dcache_dirty_new ,io.in.originDcacheDirty)
  io.out.updateDcacheShare := Mux(compare_dcwp_update_vld,update_dcache_share_new ,io.in.originDcacheShare)
  io.out.updateDcacheValid := Mux(compare_dcwp_update_vld,update_dcache_valid_new ,io.in.originDcacheValid)
