...
//    sq_entry_vld.drop(create_ptr_idx-1).reduce(_ && _) )
  ...
  for(i<- 0 until LSIQ_ENTRY){
    if(OHToUInt(age_vec_zero) == i.U){
      ...
      sq_pe_age_vec_zero_inst_type := 0.U.asTypeOf(sq_pe_age_vec_zero_inst_type)
      sq_pe_age_vec_zero_inst_size := 0.U.asTypeOf(sq_pe_age_vec_zero_inst_size)
      sq_pe_age_vec_zero_inst_mode := 0.U.asTypeOf(sq_pe_age_vec_zero_inst_mode)
      sq_pe_age_vec_zero_priv_mode := 0.U.asTypeOf(sq_pe_age_vec_zero_priv_mode)
      sq_pe_age_vec_zero_addr      := 0.U.asTypeOf(sq_pe_age_vec_zero_addr)
      sq_pe_age_vec_zero_bytes_vld := 0.U.asTypeOf(sq_pe_age_vec_zero_bytes_vld)
    }
  }
  ...
  when(sq_pe_sel_age_vec_zero_entry_vld){
    ...
    toWmbce.popAddr       := DontCare
    toWmbce.popPageCa     := DontCare
    toWmbce.popPageWa     := DontCare
    toWmbce.popPageSo     := DontCare
    toWmbce.popPageSec    := DontCare
    toWmbce.popPageBuf    := DontCare
    toWmbce.popPageShare  := DontCare
    toWmbce.popAtomic     := DontCare
    toWmbce.popIcc        := DontCare
    toWmbce.popWoSt       := DontCare
    toWmbce.popSyncFence  := DontCare
    toWmbce.popInstFlush  := DontCare
    toWmbce.popInstType   := DontCare
    toWmbce.popInstSize   := DontCare
    toWmbce.popInstMode   := DontCare
    toWmbce.popBytesVld   := DontCare
    toWmbce.popPtr        := DontCare
    toWmbce.popPrivMode   := DontCare
  }
