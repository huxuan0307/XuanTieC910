...
class ifuDataForward extends Bundle{ // @ct_iu_bju_pcfifo @1677-1684
  var curPc = UInt((PcWidth+1).W) // TODO why 40 bits?
  var dstVld = Bool()
  val en = Bool()
  var tarPc = UInt((PcWidth+1).W)
  var jal = Bool()
  var jalr = Bool()
...
class IfuPredStore extends Bundle{
  ...
  val jmpMispred = Bool()
  val pc = UInt((PcWidth+1).W) // pc ignore in forward, but use in store fifo
}
